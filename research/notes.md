...
2026-03-23 universal-chain update: under the strict "all-task applicable" criterion and fixed `10,12,12` budget, a clean additive chain from original iCaRL shows that only three components clearly survive as general continual-learning improvements: removing contrastive loss, normalized NME, and LwF. `task adapter dim=16` from stage1, global `oldweight=3.125`, `task affine` from stage1, and early `hybrid NME+logits` do not behave like universal components in this repo; they are conditional methods and should not be presented as the generic baseline pipeline.
