...
2026-03-23 fixed-budget update: after correcting the main evaluation standard back to `stage_epochs=10,12,12`, the most credible main line is `s2ace00_ptblendnew02_stage101212_confirm` (84.18/60.70/49.40). Extending asymmetric BCE from stage2 into stage3 does not help: `s2ace00_s3ace25_ptblendnew02_stage101212_short` reaches 84.72/60.96/49.61 and `s2ace00_s3ace00_ptblendnew02_stage101212_short` reaches 84.72/60.96/49.69, both below the same-budget baseline short 84.72/60.96/50.54. Current interpretation: asymmetric BCE is a valid continual-learning idea, but in this project its cleanest and strongest role is still stage2-specific missing-class handling rather than a stage-general rule.
2026-03-23 universal-chain correction: the earlier "universal methods chain" was started from an overly weak base (`mem24`, no replaymix), so its conclusion was too pessimistic. After rerunning from a more reasonable universal replay base (`no contrastive + memory=36 + replaymix2`), the corrected additive chain shows: `stage_epochs=10,12,12`, `normalized NME`, and `LwF` remain the clearest general continual-learning components; `task adapter dim=16` from stage1, global `oldweight=3.125`, `task affine` from stage1, and early `hybrid NME+logits` still do not behave like stable all-task components. The corrected chain must therefore be reported on top of the stronger replay base, not the earlier weak-base setup.
